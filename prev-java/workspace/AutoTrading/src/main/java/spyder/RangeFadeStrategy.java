...
public class RangeFadeStrategy implements Strategy
{
    ...
    private void checkSignal() {
    	...
    	if ( (currentPrice>=(todayOpen+lastRange/2)) || (currentPrice<=(todayOpen-lastRange/2)) ) {
            ...
            for (int i=0; i<2; i++) {
            	TradeAlarm.playSound("C:\\WINDOWS\\Media\\chord.wav"); 
            	try {
    		...
    		log.info("range:"+lastRange+", open"+todayOpen 
    				+",["+(todayOpen-lastRange/2)+"-"+ (todayOpen+lastRange/2) +"]");
	...
	public void monitor() {
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
        	public void run() {
        		...
        		long serverTime = dataEngine.getCurrentServerTime();  // the first few bar may not work because the servetime is not properly get. needs a few seconds. 
        		serverTime *= 1000; 
