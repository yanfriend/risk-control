...
//	OrderEngine orderEngine; 
    ...
    public DownloadData()
    {
        ...
        startTime.setTime(new Date(1970,1,1));
        endTime.setTime(new Date(2100,1,1));
    }
	...
	public void monitor() {
        ...
        if (endTime.getTime().getYear()==2100) {  // end time is not set, set it to present. 
        	endTime.setTime(now);
        }
        if ( (cal.getTime().getYear()-startTime.getTime().getYear())>1 ) {  // start time is not set, set it to 1 year ago. also starttime cannnot be >1y earlier
