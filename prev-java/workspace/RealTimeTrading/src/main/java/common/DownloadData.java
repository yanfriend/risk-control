...
//	OrderEngine orderEngine; 
	...
	public void monitor() {
		...
		if (downloadType == 0)  // 5 min, 1 year download, old input format.
			defaultDownload();    
		else if (downloadType == 1) { // new input format, daily, 6 month. 
			// "5 D", "5 mins", whatToShow, (RTH.equals("!RTH"))?0:1
			hourlyDownload(6,30, "1 M", "1 hour", 0); 
		}
    ...
    private void hourlyDownload(int howmanyMonths, final int stepDays, final String downloadPeroid, final String barSize, final int rth) {
        Date now = new Date(); 
	...
	private void saveTestData(Strategy strategy, TreeMap<Long, Bar> barMap) {
    	...
    	try {
		    BufferedWriter out = new BufferedWriter(new 
		    	FileWriter(targetLoc + strategy.getContract().m_symbol +"_"+strategy.getContract().m_expiry +".csv"));
