...
public class NppDivergenceStrategy implements Strategy  
{
    ...
    public void checkStrategy()
    {
    	...
    	if (NppDivergenceStrategy.securityNoLeft<=0) {
        ...
        float[] inClose = new float[barSize];
        float[] inHigh = new float[barSize];
        float[] inLow = new float[barSize];
        ...
        while (iterator.hasNext()) {   
        	...
        	inClose[i-1-(barCollection.size()-barSize)] = (float) bar.getClose();
        	inHigh[i-1-(barCollection.size()-barSize)] = (float) bar.getHigh();
        	inLow[i-1-(barCollection.size()-barSize)] = (float) bar.getLow();
        	
        	Date date = new Date(bar.getDate()*1000);
        	String dataStr = String.format("%d, %d:%d", date.getDate(), date.getHours(), date.getMinutes());
        	log.info("time:{}, close:{}", new Object[] {dataStr, bar.getClose()} );
        } 
        
        MInteger outBegIdx = new MInteger(); 
        MInteger outNBElement = new MInteger();
        int optInTimePeriod = 14;
        double[] outAdx = new double[barSize];
        
        taLib.adx(0, inClose.length-1, inHigh, inLow, inClose, optInTimePeriod, outBegIdx, outNBElement, outAdx);
        
        for (int num=0; num<outNBElement.value; num++) {
        	log.info("adx:{}", outAdx[num]);
        }
        
        if (outAdx[outNBElement.value-1]<30) {
        	return; 
        }
...
//    	}
	...
	private void setupLong(double[] outMACDHist,int outBegIdxMacd, int outNBElementMacd, double[] outEma, int outBegIdxEma, int outNBElementEma){
        ...
        placeOrder(order, normalizedSetupPrice, 30);
       	...
       	placed = true;
       	NppDivergenceStrategy.securityNoLeft--; 
	...
	public void monitor() {
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
        	public void run() {
        		if (currentPrice<=0) {
        			dataEngine.reqRealtimeBar(NppDivergenceStrategy.this, PriceTypeEnum.TRADES.toString(), false);
        		}
        	    ...
        	    log.info("requesting data till {}", endDateTime);
       	    	dataEngine.reqHistoricalData(NppDivergenceStrategy.this, endDateTime, "2 D", "5 mins", "TRADES", 0, 2);  // rth set to 0.    
        	}
        }, 0, period);
    }
	...
	public boolean parse(String str) {
    	...
    	try {
	    	...
	    	log.error("error in NPP file line: {}\n{}", str,e);
	    	return false; 
	    }
	}
